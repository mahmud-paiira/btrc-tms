...
class CircularForCenterAdminViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = Circular.objects.prefetch_related(
            'eligible_centers',
        ).select_related('course', 'created_by').all()
        if self.request.user.user_type == 'center_admin' and self.request.user.center:
            qs = qs.filter(Q(eligible_centers=self.request.user.center) | Q(all_centers=True))
        return qs
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(created_by=self.request.user, eligible_centers=[center.id], all_centers=False)
...
class PublicCircularViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    filterset_fields = ('eligible_centers__code', 'course', 'status')
    search_fields = ('title_bn', 'title_en')
    ...
    def get_queryset(self):
        ...
        ).prefetch_related('eligible_centers').select_related('course').all()

    @action(detail=False, methods=['get'], url_path='by-center/(?P<code>[^/.]+)')
    def by_center(self, request, code=None):
        qs = self.get_queryset().filter(Q(eligible_centers__code__iexact=code) | Q(all_centers=True))
