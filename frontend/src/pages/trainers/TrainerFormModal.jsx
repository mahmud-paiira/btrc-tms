...
export default function TrainerFormModal({ show, editData, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/trainers/${editData.id}/`, {
          ...payload,
          full_name_bn: form.full_name_bn,
          full_name_en: form.full_name_en || undefined,
          phone: form.phone,
        });
        toast.success('প্রশিক্ষকের তথ্য হালনাগাদ করা হয়েছে।');
      } else {
        const userRes = await api.post('/users/', {
          email: form.email, password: form.password || 'trainer@123',
          user_type: 'trainer', full_name_bn: form.full_name_bn,
        ...
        await api.post('/trainers/', { user: userRes.data.id, ...payload });
        toast.success('নতুন প্রশিক্ষক সফলভাবে তৈরি করা হয়েছে।');
      }
      onSaved();
      onClose();
    } catch (err) {
      ...
      if (data) {
        const msgs = Object.entries(data).map(([field, msgs]) => {
          const label = { email: 'ইমেইল', nid: 'এনআইডি', phone: 'ফোন', trainer_no: 'প্রশিক্ষক নং', date_of_birth: 'জন্ম তারিখ' }[field] || field;
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                  <h5 className="text-white mb-0 fw-bold" style={{ fontSize: 17 }}>
                    ...
                    {isEdit ? 'প্রশিক্ষক সম্পাদনা' : 'নতুন প্রশিক্ষক'}
                  </h5>
            ...
            {step === 1 && (
              <div>
                <div className="d-flex align-items-center gap-2 mb-3">
                  ...
                  <div><h6 className="fw-bold mb-0" style={{ fontSize: 14 }}>মৌলিক তথ্য</h6><small className="text-muted">প্রশিক্ষকের বেসিক তথ্য দিন</small></div>
                </div>
                    ...
                    <div className="col-md-6">
                      ...
                      <div className="input-group">
                        ...
                        <input className="form-control border-0 bg-white py-2" style={{ fontSize: 13 }}
                          value={form.email} onChange={e => set('email', e.target.value)} placeholder="trainer@brtc.gov.bd"
                          disabled={isEdit} />
                      </div>
            ...
            {step === 2 && (
              <div>
                <div className="d-flex align-items-center gap-2 mb-3">
                  ...
                  <div><h6 className="fw-bold mb-0" style={{ fontSize: 14 }}>পেশাগত তথ্য</h6><small className="text-muted">প্রশিক্ষণ সংক্রান্ত তথ্য দিন</small></div>
                </div>
