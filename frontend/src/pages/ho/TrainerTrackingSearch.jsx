...
const SEARCH_TYPES = [
  { key: 'trainer_no', label: 'প্রশিক্ষক নং' },
  { key: 'nid', label: 'এনআইডি' },
  { key: 'mobile', label: 'মোবাইল' },
  { key: 'bcn', label: 'জন্ম নিবন্ধন' },
];

export default function TrainerTrackingSearch() {
  const [searchType, setSearchType] = useState('trainer_no');
  ...
  const handleSearch = async () => {
    ...
    try {
      let res;
      switch (searchType) {
        case 'trainer_no': res = await hoService.trackTrainerByNumber(query); break;
        case 'nid': res = await hoService.trackTrainerByNid(query); break;
        case 'mobile': res = await hoService.trackTrainerByMobile(query); break;
        case 'bcn': res = await hoService.trackTrainerByBcn(query); break;
        default: return;
      }
      setResult(res.data);
    } catch (e) {
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-header bg-primary text-white py-2">
        <h6 className="mb-0 fw-semibold" style={{ fontSize: 14 }}>প্রশিক্ষক ট্র্যাকিং</h6>
      </div>
            ...
            {result ? (
              <div className="table-responsive">
                <table className="table table-bordered table-sm" style={{ fontSize: 13 }}>
                  <tbody>
                    <tr><td className="fw-semibold" style={{ width: 160 }}>প্রশিক্ষক নং</td><td>{convertToBanglaDigits(result.trainer_no)}</td></tr>
              ...
              <div className="alert alert-info py-2 mb-0" style={{ fontSize: 13 }}>
                <i className="bi bi-info-circle me-1"></i>কোনো প্রশিক্ষক পাওয়া যায়নি
              </div>
            )}
          </div>
        )}
